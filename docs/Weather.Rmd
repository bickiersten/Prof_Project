...
```{html echo=FALSE}
<div>
  <label for="dew">Enter Dew Point (°F):</label>
  <input type="number" id="dew" min="-50" max="100" value="50">
  <br>
  <label for="sea">Enter Sea Level Pressure (hPa):</label>
  <input type="number" id="sea" min="900" max="1100" value="1015">
  <br><br>
  <button onclick="predictTemperature()">Predict Temperature</button>
  <p><strong>Predicted Temperature:</strong> <span id="prediction">--</span> °F</p>
</div>
  
<script>
function predictTemperature() {
    const intercept = -36.37;
    const coefDew = 1.357;
    const coefSea = 1.595;
    ...
    const predictedTemp = intercept + coefDew * dew + coefSea * sea;

    document.getElementById("prediction").innerText = predictedTemp.toFixed(2);
  }
</script>
```
